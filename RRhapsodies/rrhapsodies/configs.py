...
DATA_PATH = {"data":
            ...
            "metadata":
                "..//data/plasticc_train_metadata.csv.gz",
             }
...
def numbers2notes(min, max):
    import sonifyFED.sonify as sonify
    import numpy as np

    notenumbers = { i+36:'/'.join(note) for i,note in enumerate(sonify.constants.NOTES)}
    for i in range(np.array(list(notenumbers.keys())).min() - 1, min-1, -1):
        notenumbers[i] = notenumbers[i + 12]
    for i in range(np.array(list(notenumbers.keys())).max() + 1, max+1):
        notenumbers[i] = notenumbers[i - 12]
