...
quantized_x = normalized_x #sthis sets the x values to integers, which aids musicality
...
normed_data = sonify.core.convert_to_key(data, key, 
number_of_octaves=noctaves, minmaxvalue=rescaling)
...
Additional note: I also implemented a couple of drones. Note that the notes can be played as rapidly as one desires. The quantize_x_value(list_to_quantize, steps=0.5)
...
By setting drone=True. Currently to 
