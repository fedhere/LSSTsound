...
def rescale(arr,new_max,new_min):
    #Rescale data to [new_min,new_max]
    arr_new = ( (arr - min(arr)) / (max(arr) - min(arr)) ) * (new_max - new_min) + new_min
    #Convert to ints
    for i in range(len(arr_new)):
        arr_new[i] = int(round(arr_new[i]))
    return int(arr_new[1])

def write_to_midifile(data, track_type='single',df=None):
    """
    data: list of tuples of x, y coordinates for pitch and timing
    ...
    fluxErrDF = df
    fluxErrorObj = fluxErrDF

    maxFluxErrRow = fluxErrorObj["flux_err"].argmax()
    maxFluxErr = fluxErrorObj["flux_err"].values[maxFluxErrRow]
    
    minFluxErrRow = fluxErrorObj["flux_err"].argmin()
    minFluxErr = fluxErrorObj["flux_err"].values[minFluxErrRow]
    ...
    for data_list in data:
        ...
        if track_type == 'single':
            fluxErrPlist = fluxErrorObj["flux_err"].to_list()

        if track_type == 'multiple':

            fluxErrFilt = fluxErrorObj["passband"].isin([S])
            fluxErrP = fluxErrorObj[fluxErrFilt]
            fluxErrPlist = fluxErrP["flux_err"].to_list()
            S+=1
        ...
        for point in data_list:
            ...
            arr = [minFluxErr, fluxErrPlist[P], maxFluxErr]
            volume = rescale(arr, 1, 100)
            
            print("Volume: " + str(volume) + " , " + " Flux Err: " + str(fluxErrPlist[P]))
    ...
    #with open("midiSound.mid", "wb") as output_file:
    #    midifile.writeFile(output_file)
    #print("Saved to disk as midiSound.mid")
    return memfile
...
def play_midi_from_data(input_data, key=None, number_of_octaves=4, track_type='single',df=None):
    """
    ...
    memfile = write_to_midifile(data, track_type,df)
